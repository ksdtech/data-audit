...
class Audit < ActiveRecord::Base
  ...
  has_many :changes, :class_name => 'AuditChange', :dependent => :delete_all
  ...
  def new_attributes
    changes.inject({}.with_indifferent_access) do |change|
      attrs[change.attribute_name] = change.new_value
      attrs
    end
  end
  ...
  def old_attributes
    changes.inject({}.with_indifferent_access) do |change|
      attrs[change.attribute_name] = change.old_value
      attrs
    end
  end
